...
var NOTES = {
  S00: '표지. `минь`(나의)를 붙여 애정을 표시했습니다.',
  S01: '기념일 9/18 확정 반영. 첫 만남의 장소·상황은 아직 모르기 때문에 정서로만 썼습니다.',
  S03: '**S24(절정)의 복선입니다.** 아내분 시를 여기서 원문 그대로 먼저 보여드려야, 나중에 답시가 나올 때 알아보십니다.',
  S05: '`гэрэл`(빛)을 여기서 심습니다. 작년에 형님이 아내분을 "내 삶의 빛"이라 부르신 그 단어이고, S24 답시의 핵심어입니다.',
  ...
  S20: '**S01(첫 만남)과 이어지는 장면입니다.** "처음 만난 날처럼 웃고 있었다"로 7년을 한 바퀴 돌립니다.',
  S22: '다음 씬(사과)으로 넘어가는 다리입니다. 형님이 서서 지켜보는 장면이 사과의 전제가 됩니다.',
...
out.push('');
out.push('2021년에 **아내분이 써주신 시**:');
out.push('');
out.push('| 아내분의 시 (2021) | 두운 |');
out.push('|---|---|');
...
out.push('');
out.push('몽골 전통 시는 유럽 시처럼 행의 끝을 맞추지 않고 **첫머리를 맞춥니다**(толгой холбох, 두운). 아내분의 시는 정확한 **Бү-Бү-Гу-Гэ** 구조입니다.');
out.push('');
out.push('형님의 답시는 **같은 음절을 되받습니다**:');
out.push('');
out.push('| 답시 (2026) | 두운 |');
out.push('|---|---|');
...
out.push('');
out.push('- 첫 행이 아내분 시의 첫 단어 `Бүрэнхий`로 시작합니다 → **아내분이 즉시 알아채는 지점**');
out.push('- 아내분은 "빛과 함께 **네가** 왔다"고 쓰셨고, 답시는 "빛을 데려온 건 내가 아니라 **너 자신이 빛이었다**"로 주어를 뒤집습니다');
...
out.push('| 위치 | 이유 |');
out.push('|---|---|');
out.push('| **S24 답시 4행** | 이 페이지에서 가장 중요한 문장입니다. 어미 하나가 어색하면 효과가 반감됩니다 |');
...
out.push('- **S14** — 아이들이 자는 동안 둘이서 일출을 봤다');
out.push('');
